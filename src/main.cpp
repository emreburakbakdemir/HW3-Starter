...
void UpdatePlanetTransforms(float time)
{
    ...
    glm::vec3 moon1Offset(
        g_planets[1].orbitRadius * glm::cos(angle1),
        0.0f,
    ...
    glm::vec3 moon2Offset(
        g_planets[2].orbitRadius * glm::cos(angle2),
        0.0f,
...
void UpdateCamera(GLState& state, GLFWwindow* wnd, float deltaTime)
{
    if (state.cameraMode == 3) // FPS mode
    {
        ...
        glm::vec3 forward(
            glm::cos(state.cameraPitch) * glm::sin(state.cameraYaw),
    ...
    else // Orbit mode (modes 0, 1, 2)
    {
        ...
        glm::vec3 offset(
            dist * glm::cos(pitch) * glm::sin(yaw),
            dist * glm::sin(pitch),
...
void KeyboardCallback(GLFWwindow* wnd, int key, int scancode, int action, int modifier)
{
    ...
    if(action == GLFW_RELEASE)
    {
        ...
        if(key == GLFW_KEY_P)
        {
            state->cameraMode = (state->cameraMode == 3) ? 0 : (state->cameraMode + 1);
        }
        if(key == GLFW_KEY_O)
        {
            state->cameraMode = (state->cameraMode == 0) ? 3 : (state->cameraMode - 1);
        }
...
int main(int argc, const char* argv[])
{
    ...
    // Load planet shaders
    ShaderGL planetVShader = ShaderGL(ShaderGL::VERTEX, "shaders/planet.vert");
    ShaderGL planetFShader = ShaderGL(ShaderGL::FRAGMENT, "shaders/planet.frag");
    ShaderGL earthFShader = ShaderGL(ShaderGL::FRAGMENT, "shaders/earth.frag");
    ShaderGL cloudFShader = ShaderGL(ShaderGL::FRAGMENT, "shaders/cloud.frag");
    // Shadow shaders
    ShaderGL shadowVShader = ShaderGL(ShaderGL::VERTEX, "shaders/shadow.vert");
    ShaderGL shadowFShader = ShaderGL(ShaderGL::FRAGMENT, "shaders/shadow.frag");
    // Background shaders
    ShaderGL bgVShader = ShaderGL(ShaderGL::VERTEX, "shaders/background.vert");
    ShaderGL bgFShader = ShaderGL(ShaderGL::FRAGMENT, "shaders/background.frag");

    // Load sphere meshes
    MeshGL sphereMesh = MeshGL("meshes/sphere_5k.obj");
    MeshGL bgSphere = MeshGL("meshes/sphere_2k.obj");

    // Load textures
    TextureGL earthTex = TextureGL("textures/2k_earth_daymap.jpg",
                                    TextureGL::LINEAR, TextureGL::REPEAT);
    TextureGL earthSpecTex = TextureGL("textures/2k_earth_specular_map.png",
                                        TextureGL::LINEAR, TextureGL::REPEAT);
    TextureGL earthNightTex = TextureGL("textures/2k_earth_nightmap_alpha.png",
                                         TextureGL::LINEAR, TextureGL::REPEAT);
    TextureGL earthCloudTex = TextureGL("textures/2k_earth_clouds_alpha.png",
                                         TextureGL::LINEAR, TextureGL::REPEAT);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, SHADOW_WIDTH, SHADOW_HEIGHT, 0, GL_RED, GL_FLOAT, nullptr);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    ...
    while(!glfwWindowShouldClose(state.window))
    {
        ...
        glUseProgramStages(state.renderPipeline, GL_VERTEX_SHADER_BIT, shadowVShader.shaderId);
        glUseProgramStages(state.renderPipeline, GL_FRAGMENT_SHADER_BIT, shadowFShader.shaderId);
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            model = glm::rotate(model, state.currentTime * g_planets[i].rotationSpeed, glm::vec3(0, 1, 0));
        ...
        glUseProgramStages(state.renderPipeline, GL_VERTEX_SHADER_BIT, bgVShader.shaderId);
        glUseProgramStages(state.renderPipeline, GL_FRAGMENT_SHADER_BIT, bgFShader.shaderId);
        ...
        // PLANET RENDERING
        // ========================================
        glUseProgramStages(state.renderPipeline, GL_VERTEX_SHADER_BIT, planetVShader.shaderId);
        glBindVertexArray(sphereMesh.vaoId);
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            if (i == 0)
            {
                ...
                glUseProgramStages(state.renderPipeline, GL_FRAGMENT_SHADER_BIT, earthFShader.shaderId);
            ...
            else
            {
                ...
                glUseProgramStages(state.renderPipeline, GL_FRAGMENT_SHADER_BIT, planetFShader.shaderId);
        ...
        // Render Earth clouds separately
        {
            ...
            cloudModel = glm::rotate(cloudModel, state.currentTime * cloudRotationSpeed, glm::vec3(0, 1, 0));
            cloudModel = glm::scale(cloudModel, glm::vec3(g_planets[0].scale * cloudScale));

            glm::mat3x3 cloudNormalMatrix = glm::inverseTranspose(glm::mat3(cloudModel));
            ...
            glUniformMatrix4fv(U_TRANSFORM_VIEW, 1, false, glm::value_ptr(view));
            glUniformMatrix4fv(U_TRANSFORM_PROJ, 1, false, glm::value_ptr(proj));
            glUniformMatrix3fv(U_TRANSFORM_NORMAL, 1, false, glm::value_ptr(cloudNormalMatrix));

            // Use cloud shader
            glUseProgramStages(state.renderPipeline, GL_FRAGMENT_SHADER_BIT, cloudFShader.shaderId);
